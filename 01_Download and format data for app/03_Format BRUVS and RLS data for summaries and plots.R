...
region_top_species_average <- combined_count %>%
  ...
  dplyr::select(
    campaignid, sample, region, period,
    family, genus, species, genus_species, count
  ) %>%
  tidyr::complete(
    tidyr::nesting(campaignid, sample, region, period),
    tidyr::nesting(family, genus, species, genus_species)
  ) %>%
  dplyr::filter(!is.na(species)) %>%
...
region_top_species_average_status <- combined_count %>%
  ...
  dplyr::select(
    ...
    genus, species, family, genus_species, count
  ) %>%
  tidyr::complete(
  ...
  dplyr::filter(!is.na(species)) %>%
  tidyr::replace_na(list(count = 0)) %>%
  dplyr::group_by(region, period, status, family, genus, species, genus_species) %>%
  dplyr::summarise(
    average = mean(count, na.rm = TRUE),
    se      = sd(count, na.rm = TRUE) / sqrt(sum(!is.na(count))),
    n_samples_present = sum(count > 0, na.rm = TRUE),
...
location_top_species_average_status <- combined_count %>%
  ...
  dplyr::filter(!is.na(species)) %>%
  tidyr::replace_na(list(count = 0)) %>%
  dplyr::group_by(reporting_name, period, status, family, genus, species, genus_species) %>%
  dplyr::summarise(
    average = mean(count, na.rm = TRUE),
    se      = sd(count, na.rm = TRUE) / sqrt(sum(!is.na(count))),
    n_samples_present = sum(count > 0, na.rm = TRUE),
...
location_top_species_average <- combined_count %>%
  ...
  # keep region/period/status here so we don't have to join later
  dplyr::select(
    campaignid, sample, reporting_name, period,
    family, genus, species, genus_species, count
  ) %>%
  tidyr::complete(
    tidyr::nesting(campaignid, sample, reporting_name, period),
    tidyr::nesting(family, genus, species, genus_species)
  ) %>%
  dplyr::filter(!is.na(species)) %>%
  tidyr::replace_na(list(count = 0)) %>%
  dplyr::group_by(reporting_name, period, family, genus, species, genus_species) %>%
  dplyr::summarise(
    average = mean(count, na.rm = TRUE),
    se      = sd(count, na.rm = TRUE) / sqrt(sum(!is.na(count))),
    n_samples_present = sum(count > 0, na.rm = TRUE),
...
trophic_groups_samples <- combined_count %>%
  ...
  dplyr::mutate(diet = if_else(diet %in% c("NA", NA), "Diet unknown", diet)) %>%
...
hab_data <- structure(
  list(
    ...
    region_top_species_average_status = region_top_species_average_status,
    
    location_top_species_average = location_top_species_average,
    location_top_species_average_status = location_top_species_average_status,
