...
m1_fish_shannon_blocks <- read_rds("data/tidy/rls_m1_complete_count.rds") %>%
...
m2_fish_shannon_blocks <- read_rds("data/tidy/rls_m2_fish_complete_count.rds") %>%
...
m2_inverts_shannon_blocks <- read_rds("data/tidy/rls_m2_inverts_complete_count.rds") %>%
...
m2_fish_site_shannon_average <- m2_fish_shannon_samples %>%
  ungroup() %>%
  dplyr::group_by(site_name, site_code, sampling_event, latitude, longitude, 
                  period, period_split, sampling_event_start_date) %>%
  dplyr::summarise(mean = mean(shannon, na.rm = TRUE),
...
m2_inverts_site_shannon_average <- m2_inverts_shannon_samples %>%
  ungroup() %>%
  dplyr::group_by(site_name, site_code, sampling_event, latitude, longitude, 
                  period, period_split, sampling_event_start_date) %>%
  dplyr::summarise(mean = mean(shannon, na.rm = TRUE),
...
## Save formatted data ----
write_rds(m1_fish_site_shannon_average, "data/tidy/rls_m1_fish_shannon_average_per_site.rds")
write_rds(m2_fish_site_shannon_average, "data/tidy/rls_m2_fish_shannon_average_per_site.rds")
write_rds(m2_inverts_site_shannon_average, "data/tidy/rls_m2_inverts_shannon_average_per_site.rds")

# Creating summarised values for period and for period split -----
summarise_site_diversity <- function(data, period_variable) {
...
## Period comparisons ----
m1_fish_site_period <- m1_fish_shannon_samples %>%
  summarise_site_diversity(period)

m2_fish_site_period <- m2_fish_shannon_samples %>%
  summarise_site_diversity(period)

m2_inverts_site_period <- m2_inverts_shannon_samples %>%
  summarise_site_diversity(period)

## Multiple Period comparisons ----
m1_fish_site_period_split <- m1_fish_shannon_samples %>%
  summarise_site_diversity(period_split)

m2_fish_site_period_split <- m2_fish_shannon_samples %>%
  summarise_site_diversity(period_split)

m2_inverts_site_period_split <- m2_inverts_shannon_samples %>%
  summarise_site_diversity(period_split)

# Plot observed diversity by site ----
##  Plot settings ----
plot_output_root <- file.path(
  "plots",
  "rls_shannon_observed"
)
...
temporal_plot_data <- dplyr::bind_rows(
  ...
  m2_inverts_site_shannon_average %>%
    dplyr::transmute(
      site_name,
      site_code = as.character(site_code),
      sampling_event,
