...
add_spatial_lookup <- function(data, lookup) {
  ...
  data %>%
    ...
    dplyr::select(
      -site_code_from_name,
      -dplyr::any_of(c("location_g", "region"))
    ) %>%
    dplyr::left_join(
      lookup_by_code,
      by = "site_code"
...
sa_sites <- sf::read_sf(
  ...
  dplyr::transmute(
    ...
    location_g = as.character(location_g),
...
m1_b20_samples <- m1_b20$samples # 1821
m2_b20_samples <- m2_b20$samples # 1837

# Combine M1 and M2 now, so all later operations are performed once.
b20_samples <- dplyr::bind_rows(
  m1_b20_samples,
  m2_b20_samples
) %>%
  add_spatial_lookup(sa_sites) %>%
  dplyr::mutate(
...
unmatched_spatial_rows <- b20_samples %>%
  dplyr::filter(is.na(location_g) | is.na(region)) %>%
  dplyr::distinct(site_code, site_name, location_g, region)
