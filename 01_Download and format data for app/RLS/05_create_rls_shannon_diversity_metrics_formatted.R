...
summarise_shannon <- function(data, group_vars) {
...
m1_fish <- prepare_shannon_dataset(
...
readr::write_rds(
  shannon_samples,
  "data/rls_metrics_for_modelling/shannon_diversity.rds"
)
...
spatial_samples <- expand_spatial_levels(shannon_samples)
...
plot_log <- purrr::pmap_dfr(
  ...
  function(spatial_level, group_id, group_name, file_stub, safe_id) {
    ...
    tryCatch(
      {
        plots_saved <- save_shannon_plots(
