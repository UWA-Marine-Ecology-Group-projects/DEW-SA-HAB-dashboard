#################################################################
# Create and plot observed Simpson diversity from RLS M1 and M2 data
#
# Metrics:
#   - M1 fish Simpson diversity
#   - M2 fish Simpson diversity
#   - M2 invertebrate Simpson diversity
#   - M2 invertebrate Simpson diversity for each phylum in
...
# that phylum is retained as Simpson = 0, which is the same convention the
# whole-dataset calculation already uses for an empty block.
#
# Simpson diversity is reported as the inverse Simpson index, 1 / sum(p^2)
# (vegan "invsimpson"): 1 for a single species, up to the number of
# species when all are equally abundant; higher values mean higher
# diversity. Empty blocks are set to 0. Compared with Shannon it gives
# more weight to the dominant species and is less sensitive to rare
# species. This script mirrors script 05 (Shannon)
# exactly apart from the index, and writes to its own files/folders.
#
# Note when interpreting these: Simpson diversity within a single phylum
# is bounded by how many species of that phylum the method records, so a
# phylum with only a handful of species will sit low (near 1, or 0 where
# the phylum is absent) for reasons of
...
#   "M2 invertebrate <phylum> Simpson diversity"
invert_phylum_simpson_metric_name <- function(phylum) {
  paste0("M2 invertebrate ", phylum, " Simpson diversity")
}
...
invert_phylum_metric_levels <- invert_phylum_simpson_metric_name(
  target_invert_phyla
)
...
plot_output_roots <- c(
  # site = file.path("plots", "rls_simpson_diversity_site"),
  location = file.path("plots", "rls_simpson_diversity_location"),
  region = file.path("plots", "rls_simpson_diversity_region")
)
...
phylum_plot_output_roots <- c(
  location = file.path("plots", "rls_simpson_diversity_phyla_location"),
  region = file.path("plots", "rls_simpson_diversity_phyla_region")
)
...
# Inverse (reciprocal) Simpson index of one block's abundances:
#
#   1 / D,  where D = sum(p_i^2) and p_i = n_i / N
#
# This is the same calculation as vegan::diversity(x, index = "invsimpson").
# It can be read as the "effective number of equally common species":
# it equals 1 when only one species is present and equals the number of
# species when they are all equally abundant, so higher values mean higher
# diversity. The p^2 form is used rather than n(n - 1) / N(N - 1) because
# the latter gives an infinite value whenever every species in a block is
# a singleton, which is common in small RLS blocks.
#
# An empty block (no individuals recorded) returns 0, matching the
# convention used in the Shannon script. Used for both the whole-dataset
# and per-phylum versions so they cannot drift apart.
simpson_index <- function(total) {
...
# Calculates Simpson diversity of individual blocks to then be averaged ----
...
calculate_block_diversity <- function(data, metric_name) {
  ...
  data %>%
    ...
    dplyr::summarise(
      simpson = simpson_index(total),
      .groups = "drop"
    ) %>%
    dplyr::mutate(metric = metric_name)
}

# Calculates block-level Simpson diversity separately for each requested
# phylum.
#
...
calculate_block_diversity_by_phylum <- function(
  ...
  if (!"phylum" %in% names(data)) {
    stop(
      dataset_name,
      " does not contain a `phylum` column, so phylum-specific Simpson ",
  ...
  block_keys %>%
    ...
    dplyr::mutate(
      ...
      metric = invert_phylum_simpson_metric_name(phylum)
    ) %>%
    dplyr::select(transect, block, metric, simpson)
}

# Performs the repeated count-file -> block Simpson -> transect Simpson steps.
#
# `phyla` is optional. When supplied, phylum-specific Simpson metrics are
# calculated from the same (already cleaned) count data and returned in the
# same long table, one row per transect x metric.
prepare_simpson_dataset <- function(
    count_path,
    survey_list,
    dataset_name,
    metric_name,
  ...
  if (!is.null(phyla) && length(phyla) > 0) {
    ...
    metric_names <- c(
      metric_names,
      invert_phylum_simpson_metric_name(phyla)
    )
  }
  ...
  transect_diversity <- block_diversity %>%
    ...
    dplyr::summarise(
      ...
      block_sd = stats::sd(simpson, na.rm = TRUE),
      n_blocks = dplyr::n_distinct(block),
      .groups = "drop"
    ) %>%
    dplyr::rename(simpson = mean_simpson)
  ...
  # Simpson = 0. With more than one metric per transect that has to become
  ...
  sample_diversity <- tidyr::expand_grid(
    ...
    tidyr::replace_na(list(simpson = 0)) %>%
...
# Convert the transect table into common site, location and region columns.
# Sites are summarised by sampling event; locations and regions by month.
...
# Generic mean, SD, SE and transect-count summary for any grouping columns.
summarise_simpson <- function(data, group_vars) {
...
plot_observed_period <- function(data) {
  ...
  ggplot(
    ...
    labs(
      x = NULL,
      y = "Average inverse Simpson index\n(± SE)",
      fill = NULL
    ) +
...
# -----------------------------------------------------------------
# 5. Calculate Simpson diversity for the three datasets
# -----------------------------------------------------------------

m1_fish <- prepare_simpson_dataset(
...
m2_fish <- prepare_simpson_dataset(
...
# only one asked for phylum-specific Simpson diversity.
m2_inverts <- prepare_simpson_dataset(
...
simpson_samples <- dplyr::bind_rows(
  m1_fish$samples,
  m2_fish$samples,
  m2_inverts$samples
) %>%
...
readr::write_rds(
  simpson_samples,
  "data/rls_metrics_for_modelling/simpson_diversity.rds"
)
...
spatial_samples <- expand_spatial_levels(simpson_samples)
...
site_average_paths <- c(
  "M1 fish Simpson diversity" =
    "data/tidy/rls_m1_fish_simpson_average_per_site.rds",
  "M2 fish Simpson diversity" =
    "data/tidy/rls_m2_fish_simpson_average_per_site.rds",
  "M2 invertebrate Simpson diversity" =
    "data/tidy/rls_m2_inverts_simpson_average_per_site.rds"
)
...
save_simpson_plot_family <- function(
    spatial_level_value,
    group_id_value,
    safe_id_value,
    plot_family) {
...
save_simpson_plots <- function(
  ...
  saved <- purrr::map(
    ...
    function(plot_family) {
      ...
      family_saved <- save_simpson_plot_family(
  ...
  # reaching this loop should have at least one Simpson diversity metric.
  if (length(saved) == 0) {
...
plot_log <- purrr::pmap_dfr(
  ...
  function(spatial_level, group_id, group_name, file_stub, safe_id) {
    ...
    tryCatch(
      {
        plots_saved <- save_simpson_plots(
