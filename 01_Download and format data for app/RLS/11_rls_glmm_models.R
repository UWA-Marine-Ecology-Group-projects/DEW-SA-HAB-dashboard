...
#
# Spatial outputs:
#   - Location only: GLMMs are fitted separately to each location.
...
#     strongly to the lower Tweedie boundary rather than an interior optimum.
#   - B20: Tweedie with log link; Tweedie power is still estimated freely
#     within each location model until B20 is profiled separately.
#
...
# ============================================================
# 4. Estimate ONE pooled Tweedie power for each abundance metric
# ============================================================
#
# These pooled models are used ONLY to estimate the distributional Tweedie
# power parameter p. They do not replace the separate location-level models.
#
# The same p is then held fixed for every location model belonging to that
# abundance metric. This avoids estimating p separately in every location.
#
# Arthropoda is treated separately because its detailed AIC profile showed a
# very strong preference for p -> 1 rather than an interior optimum.

pooled_abundance_metrics <- c(
  "M1 fish total abundance",
  "M2 fish total abundance",
  "M2 invertebrate total abundance",
  "M2 invertebrate Echinodermata abundance",
...
arthropoda_fixed_power <- 1.001

estimate_pooled_power <- function(metric_name) {

  dat <- all_dat %>%
    filter(as.character(metric) == metric_name) %>%
    mutate(
      Location = factor(location),
      Site = interaction(location, site_code, drop = TRUE),
      SamplingEvent = interaction(
        location, sampling_event_start_date, drop = TRUE
      )
    )
  ...
  if (inherits(fit, "error")) {
    return(
      tibble(
        ...
        pdHess = FALSE,
        warnings = if (length(captured_warnings) > 0) {
  ...
  tibble(
    metric = metric_name,
    power = unname(glmmTMB::family_params(fit)["Tweedie power"]),
...
# Estimate p from pooled data for the five abundance metrics with clear
# interior optima in the earlier AIC profiles.
pooled_abundance_p <- purrr::map_dfr(
  pooled_abundance_metrics,
  estimate_pooled_power
)

print(pooled_abundance_p, n = Inf)

# Stop before the location-level run if a pooled p was not estimated cleanly.
invalid_pooled_p <- pooled_abundance_p %>%
  filter(
    is.na(power) |
      !is.finite(power) |
      power <= 1 |
...
if (nrow(invalid_pooled_p) > 0) {
  print(invalid_pooled_p, n = Inf)
  stop(
    "At least one pooled abundance Tweedie power was not estimated cleanly. ",
    "Inspect invalid_pooled_p before fitting the location models."
  )
}

# Final fixed-p table used by the location models.
# Arthropoda is fixed at p = 1.001 from its lower-boundary AIC profile.
abundance_fixed_p <- pooled_abundance_p %>%
  select(metric, fixed_tweedie_power = power) %>%
  bind_rows(
    tibble(
      metric = "M2 invertebrate Arthropoda abundance",
      fixed_tweedie_power = arthropoda_fixed_power
    )
  )

missing_fixed_p <- setdiff(
  abundance_metrics_to_model,
  abundance_fixed_p$metric
)

duplicate_fixed_p <- abundance_fixed_p %>%
  count(metric, name = "n") %>%
  filter(n != 1)

if (length(missing_fixed_p) > 0) {
  stop(
    "Missing fixed Tweedie power for: ",
    paste(missing_fixed_p, collapse = ", ")
  )
}

if (nrow(duplicate_fixed_p) > 0) {
  stop("At least one abundance metric has more than one fixed Tweedie power.")
}

print(abundance_fixed_p, n = Inf)

# Add the selected p to every row. Non-abundance metrics, including B20,
# receive NA and therefore retain their existing family behaviour.
fixed_p_lookup <- stats::setNames(
  abundance_fixed_p$fixed_tweedie_power,
  abundance_fixed_p$metric
)

all_dat <- all_dat %>%
  mutate(
    fixed_tweedie_power = unname(
      fixed_p_lookup[as.character(metric)]
    )
  )


# ============================================================
# 6. Data checks before fitting models
# ============================================================

# 6a. Responses must be non-negative for all selected metrics.
...
fit_glmmTMB_safely <- function(formula, data, family_object) {
  ...
  # Pull the fixed p carried by this metric's rows.
  fixed_p_values <- if ("fixed_tweedie_power" %in% names(data)) {
    unique(data$fixed_tweedie_power[is.finite(data$fixed_tweedie_power)])
  } else {
    numeric()
  }

  if (length(fixed_p_values) > 1) {
    stop("More than one fixed Tweedie power was found within a model dataset.")
  }

  fixed_tweedie_power <- if (length(fixed_p_values) == 1) {
    fixed_p_values[[1]]
  } else {
    NA_real_
  }
  ...
  if (
    is.finite(fixed_tweedie_power) &&
      !(fixed_tweedie_power > 1 && fixed_tweedie_power < 2)
  ) {
    stop("Fixed Tweedie power must be strictly between 1 and 2.")
  }

  # Use only TWO optimiser attempts:
  #   1. default nlminb;
  #   2. BFGS if the default fit is not valid.
  #
  # If fixed_tweedie_power is present, BOTH attempts keep p fixed.
  fit_once <- function(control_object, optimiser_label) {
    ...
    if (is.finite(fixed_tweedie_power)) {
      fit_args$start <- list(
        psi = qlogis(fixed_tweedie_power - 1)
      )
      fit_args$map <- list(
        psi = factor(NA)
      )
    ...
    list(
      ...
      convergence_message = tryCatch(
        as.character(fit$fit$message),
        error = function(e) NA_character_
      )
    )
  }

  attempt_default <- fit_once(
    control_object = glmmTMB::glmmTMBControl(),
    optimiser_label = "nlminb_default"
  )

  check_default <- assess_model_fit(attempt_default$model)

  if (check_default$valid) {
    message("    Default nlminb successful")
    return(attempt_default)
  }

  message("    Default nlminb unsuccessful - trying BFGS...")

  attempt_bfgs <- fit_once(
    control_object = glmmTMB::glmmTMBControl(
    ...
    optimiser_label = "optim_BFGS"
  )

  check_bfgs <- assess_model_fit(attempt_bfgs$model)

  if (check_bfgs$valid) {
    message("    BFGS successful")
    return(attempt_bfgs)
  }

  message("    BFGS also unsuccessful - model will be flagged")

  attempts <- list(attempt_default, attempt_bfgs)
  checks <- list(check_default, check_bfgs)

  scores <- purrr::map_dbl(
    checks,
    ~ 8 * isTRUE(.x$pdHess) +
      4 * isTRUE(.x$finite_standard_errors) +
      2 * is.finite(.x$AIC) +
      1 * isTRUE(.x$converged)
  )

  best_index <- which.max(scores)
  best <- attempts[[best_index]]

  best$warnings <- combine_messages(
    paste0("nlminb_default: ", attempt_default$warnings),
    paste0("optim_BFGS: ", attempt_bfgs$warnings)
  )
...
make_model_diagnostic <- function(
  ...
  tibble(
    ...
    optimizer_used = fit_result$optimizer,
...
fit_one_location_metric <- function(df) {
  ...
  fixed_p_here <- unique(
    df$fixed_tweedie_power[is.finite(df$fixed_tweedie_power)]
  )

  if (length(fixed_p_here) == 1) {
    message("Tweedie p: fixed at ", signif(fixed_p_here[[1]], 6))
  } else if (family_code == "tweedie") {
    message("Tweedie p: estimated freely")
  }
  ...
  if (all(df$response == 0, na.rm = TRUE)) {
    ...
    # Add the same identifiers used by model-based outputs so all-zero
    # location/metric combinations are retained as genuine sampled zeros.
...
# ============================================================
# 13. Prepare LOCATION-only prediction tables
# ============================================================

# This version deliberately stops at the location level.
# No regional averaging or regional prediction tables are calculated.

period_results <- location_period_results %>%
  mutate(
    Period = factor(Period, levels = period_levels),
    metric = factor(metric, levels = levels(all_dat$metric))
  ) %>%
  arrange(group_name, metric, Period)

period_status_results <- location_period_status_results %>%
  mutate(
    Period = factor(Period, levels = period_levels),
    status = factor(status, levels = status_levels),
    metric = factor(metric, levels = levels(all_dat$metric))
  ) %>%
  arrange(group_name, metric, Period, status)

temporal_results <- location_temporal_results %>%
  mutate(
    Period = factor(Period, levels = period_levels),
    metric = factor(metric, levels = levels(all_dat$metric))
  ) %>%
...
readr::write_excel_csv(
  abundance_fixed_p,
  file.path(model_output_root, "abundance_tweedie_p_used.csv")
)

readr::write_excel_csv(
  pooled_abundance_p,
  file.path(model_output_root, "pooled_abundance_tweedie_p_estimates.csv")
)
...
writexl::write_xlsx(
  list(
    ...
    abundance_tweedie_p = abundance_fixed_p,
    pooled_tweedie_p = pooled_abundance_p
  ),
...
plot_period_status_prediction <- function(df) {
  ...
  # Asterisk marks Status / Period x Status estimates with low site replication.
  plot_df <- df %>%
    mutate(
...
# ============================================================
# 17. Useful summary checks after the run
# ============================================================
...
period_status_results %>%
  count(low_replication) %>%
  print(n = Inf)


# ============================================================
# 18. Optional DHARMa diagnostic helper
# ============================================================
