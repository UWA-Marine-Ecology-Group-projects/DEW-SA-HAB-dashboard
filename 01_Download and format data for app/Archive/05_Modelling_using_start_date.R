...
unique(abund_dat$start_date)
unique(abund_dat$start_month)
...
fit_one_reporting_name <- function(df,
  ...
  if (
    nrow(df) < 10 ||
    n_distinct(droplevels(df$start_date)) < 2
  ) {
...
run_metric_models <- function(df,
  ...
  for (area_name in names(split_dat)) {
    ...
    message("Rows per start_date:")
    print(table(area_df$start_date))
    ...
    message("start_dates: ", n_distinct(area_df$start_date), " | ", paste(sort(unique(area_df$start_date)), collapse = ", "))
    ...
    result <- tryCatch(
      {
        ...
        list(
          ...
          diagnostics = tibble(
            ...
            metric = metric_name,
            n_rows = nrow(area_df),
            n_start_dates = n_distinct(area_df$start_date),
...
# # MODELLING BY start_date ----
# 
# # -----------------------------
# # 1. Prepare data with numeric start_date
# # -----------------------------
# 
# prep_metric_data_start_date <- function(df, response_col) {
#   df %>%
#     filter(
#       !is.na(.data[[response_col]]),
#       !is.na(status),
...
#     mutate(
#       Status = factor(status, levels = c("Fished", "No-take")),
#       # start_date = as.numeric(as.character(start_date)),
#       # start_date_scaled = as.numeric(scale(start_date)),
#       start_date = factor(start_date),
#       site = factor(sample)
#     )
# }
# 
# abund_dat <- prep_metric_data_start_date(
#   hab_data$total_abundance_samples,
#   "total_abundance_sample"
# )
# 
# rich_dat <- prep_metric_data_start_date(
#   hab_data$species_richness_samples,
#   "n_species_sample"
# )
# 
# shark_dat <- prep_metric_data_start_date(
#   hab_data$shark_ray_richness_samples %>% left_join(metadata),
#   "n_species_sample"
# )
# 
# reef_dat <- prep_metric_data_start_date(
#   hab_data$reef_associated_richness_samples %>% left_join(metadata),
#   "n_species_sample"
# )
# 
# shannon_dat <- prep_metric_data_start_date(
#   hab_data$shannon_diversity_samples %>% left_join(metadata),
#   "shannon")  %>%
#   glimpse()
# 
# fish_200_dat <- prep_metric_data_start_date(
#   hab_data$fish_200_abundance_samples %>% left_join(metadata),
#   "total_abundance_sample")  %>%
#   glimpse()
...
# MODELLING BY start_date AS FIXED FACTOR ----
...
prep_metric_data_start_date_fixed <- function(df, response_col) {
  df %>%
    filter(
      !is.na(.data[[response_col]]),
      !is.na(period),
...
abund_start_date_dat <- prep_metric_data_start_date_fixed(
  hab_data$total_abundance_samples,
  "total_abundance_sample"
)

rich_start_date_dat <- prep_metric_data_start_date_fixed(
  hab_data$species_richness_samples,
  "n_species_sample"
)

shark_start_date_dat <- prep_metric_data_start_date_fixed(
  hab_data$shark_ray_richness_samples %>% left_join(metadata),
  "n_species_sample"
)

reef_start_date_dat <- prep_metric_data_start_date_fixed(
  hab_data$reef_associated_richness_samples %>% left_join(metadata),
  "n_species_sample"
)

shannon_start_date_dat <- prep_metric_data_start_date_fixed(
  hab_data$shannon_diversity_samples %>% left_join(metadata),
  "shannon"
)

fish_200_start_date_dat <- prep_metric_data_start_date_fixed(
  hab_data$fish_200_abundance_samples %>% left_join(metadata),
  "total_abundance_sample"
)
...
fit_one_reporting_name_start_date_fixed <- function(df,
                                              response_col,
                                              metric_name,
                                              use_site = FALSE) {
  ...
  if (
    ...
    stop("Not enough data to fit start_date model")
  }
  ...
  if (has_two_status) {
    ...
    start_date_status_means <- start_date_means %>%
  ...
  start_date_status_means <- start_date_status_means %>%
    mutate(
      ...
      summary_type = ifelse(
        has_two_status,
        "start_date by Status",
        "start_date only - single Status"
      )
    )
...
run_metric_start_date_fixed_models <- function(df,
                                         response_col,
                                         metric_name,
                                         use_site = FALSE,
                                         output_dir = "model_outputs_start_date_fixed",
                                         overwrite = FALSE) {
  ...
  for (area_name in names(split_dat)) {
    ...
    result <- tryCatch(
      {
        fit <- fit_one_reporting_name_start_date_fixed(
          area_df,
          response_col,
          metric_name,
          use_site = use_site
  ...
  start_date_means <- map_dfr(
    all_outputs,
    ~ if (is.null(.x$error) && !is.null(.x$result)) .x$result$start_date_means else NULL
  )
  
  start_date_status_means <- map_dfr(
    all_outputs,
    ~ if (is.null(.x$error) && !is.null(.x$result)) .x$result$start_date_status_means else NULL
  )
...
abund_start_date_fixed_models <- run_metric_start_date_fixed_models(
  abund_start_date_dat,
  response_col = "total_abundance_sample",
  metric_name = "Total abundance",
  use_site = TRUE,
  output_dir = "model_outputs_start_date_fixed/abundance"
)

rich_start_date_fixed_models <- run_metric_start_date_fixed_models(
  rich_start_date_dat,
  response_col = "n_species_sample",
  metric_name = "Species richness",
  use_site = FALSE,
  output_dir = "model_outputs_start_date_fixed/species_richness"
)

shark_start_date_fixed_models <- run_metric_start_date_fixed_models(
  shark_start_date_dat,
  response_col = "n_species_sample",
  metric_name = "Shark and ray richness",
  use_site = FALSE,
  output_dir = "model_outputs_start_date_fixed/shark_ray_richness"
)

reef_start_date_fixed_models <- run_metric_start_date_fixed_models(
  reef_start_date_dat,
  response_col = "n_species_sample",
  metric_name = "Reef associated species richness",
  use_site = FALSE,
  output_dir = "model_outputs_start_date_fixed/reef_associated_richness"
)

shannon_start_date_fixed_models <- run_metric_start_date_fixed_models(
  shannon_start_date_dat,
  response_col = "shannon",
  metric_name = "Shannon diversity",
  use_site = FALSE,
  output_dir = "model_outputs_start_date_fixed/shannon"
)

fish_200_start_date_fixed_models <- run_metric_start_date_fixed_models(
  fish_200_start_date_dat,
  response_col = "total_abundance_sample",
  metric_name = "Abundance > 200 mm",
  use_site = FALSE,
  output_dir = "model_outputs_start_date_fixed/200mm"
)
...
start_date_results <- bind_rows(
  abund_start_date_fixed_models$start_date_means,
  rich_start_date_fixed_models$start_date_means,
  shark_start_date_fixed_models$start_date_means,
  reef_start_date_fixed_models$start_date_means,
  shannon_start_date_fixed_models$start_date_means,
  fish_200_start_date_fixed_models$start_date_means
)

start_date_status_results <- bind_rows(
  abund_start_date_fixed_models$start_date_status_means,
  rich_start_date_fixed_models$start_date_status_means,
  shark_start_date_fixed_models$start_date_status_means,
  reef_start_date_fixed_models$start_date_status_means,
  shannon_start_date_fixed_models$start_date_status_means,
  fish_200_start_date_fixed_models$start_date_status_means
)

start_date_model_errors <- bind_rows(
  abund_start_date_fixed_models$model_errors,
  rich_start_date_fixed_models$model_errors,
  shark_start_date_fixed_models$model_errors,
  reef_start_date_fixed_models$model_errors,
  shannon_start_date_fixed_models$model_errors,
  fish_200_start_date_fixed_models$model_errors
)

start_date_model_diagnostics <- bind_rows(
  abund_start_date_fixed_models$model_diagnostics,
  rich_start_date_fixed_models$model_diagnostics,
  shark_start_date_fixed_models$model_diagnostics,
  reef_start_date_fixed_models$model_diagnostics,
  shannon_start_date_fixed_models$model_diagnostics,
  fish_200_start_date_fixed_models$model_diagnostics
)

saveRDS(start_date_results, "model_outputs_start_date_fixed/start_date_results.rds")
saveRDS(start_date_status_results, "model_outputs_start_date_fixed/start_date_status_results.rds")
saveRDS(start_date_model_errors, "model_outputs_start_date_fixed/start_date_model_errors.rds")
saveRDS(start_date_model_diagnostics, "model_outputs_start_date_fixed/start_date_model_diagnostics.rds")
...
start_date_results <- start_date_results %>%
  mutate(
    start_date_num = as.numeric(as.character(start_date)),
    Period = factor(Period, levels = c("Pre-bloom", "Bloom")),
    metric_id = recode(metric, !!!metric_lookup)
  ) %>%
  arrange(reporting_name, metric_id, start_date_num)
...
# -----------------------------
# 7. Plot by start_date AND status
# -----------------------------
...
plot_one_metric_start_date_status_bar <- function(df, metric_id, panel_letter) {
  ...
  ggplot(
    metric_df,
    aes(x = start_date_num, y = response, fill = Status)
  ) +
    geom_col(
    ...
    scale_x_continuous(
      breaks = sort(unique(metric_df$start_date_num)),
      labels = sort(unique(metric_df$start_date_num)),
...
dir.create(
  "plots/start_date_status_results_fixed",
  recursive = TRUE,
  showWarnings = FALSE
)

for (region in unique(start_date_status_results$reporting_name)) {
  
  message("Plotting start_date status bars: ", region)
  
  plot_df <- start_date_status_results %>%
    filter(reporting_name == region)
  
  plots <- purrr::map2(
    metric_order,
    LETTERS[seq_along(metric_order)],
    ~ plot_one_metric_start_date_status_bar(plot_df, .x, .y)
  )
  
  p <- wrap_plots(plots, ncol = 2, guides = "collect") +
    plot_annotation(title = paste(region, "- start_dately results by status")) &
    theme(
      plot.title = element_text(size = 18, hjust = 0.5),
      legend.position = "bottom"
    )
  ...
  ggsave(
    filename = file.path(
      "plots/start_date_status_results_fixed",
      paste0(safe_name, "_start_date_status_bars.png")
    ),
    plot = p,
    width = 8,
    height = 10,
...
# -----------------------------
# 8. Plot one bar per start_date, coloured by Period
# -----------------------------

plot_one_metric_start_date_period_bar <- function(df, metric_id, panel_letter) {
  ...
  ggplot(
    metric_df,
    aes(x = start_date_num, y = response, fill = Period)
  ) +
    geom_col(
      width = 0.65,
      colour = "black",
...
dir.create(
  "plots/start_date_period_results_fixed",
  recursive = TRUE,
  showWarnings = FALSE
)

for (region in unique(start_date_results$reporting_name)) {
  
  message("Plotting one bar per start_date: ", region)
  
  plot_df <- start_date_results %>%
    filter(reporting_name == region)
  
  plots <- purrr::map2(
    metric_order,
    LETTERS[seq_along(metric_order)],
    ~ plot_one_metric_start_date_period_bar(plot_df, .x, .y)
  )
  
  p <- wrap_plots(plots, ncol = 2, guides = "collect") +
    plot_annotation(title = paste(region, "- start_dately results")) &
    theme(
      plot.title = element_text(size = 18, hjust = 0.5),
      legend.position = "bottom"
    )
  ...
  ggsave(
    filename = file.path(
      "plots/start_date_period_results_fixed",
      paste0(safe_name, "_start_date_period_bars.png")
    ),
    plot = p,
    width = 8,
    height = 10,
