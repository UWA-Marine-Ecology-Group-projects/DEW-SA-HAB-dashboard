...
# Gaussian models for species-richness metrics
# Tweedie models for all other metrics
# ============================================================
...
# Richness metrics use Gaussian models with an log link.
# All other metrics use Tweedie models with a log link.
analysis_tag <- "20260728_gaussian_tweedie"
...
#   "Species richness",                      "richness",                        "gaussian",   "Gaussian (log link)",    "log",
#   "Shark and ray richness",                "sharks_rays",                     "gaussian",   "Gaussian (log link)",    "log",
#   "Reef associated species richness",      "reef_associated_richness",         "gaussian",   "Gaussian (log link)",    "log",
#   "Total abundance",                       "total_abundance",                  "tweedie",    "Tweedie (log link)",          "log",
#   "Abundance > 200 mm",                    "large_fish",                       "tweedie",    "Tweedie (log link)",          "log",
#   "Shannon diversity",                     "shannon_diversity",                "tweedie",    "Tweedie (log link)",          "log"
# )
...
#         label = "Gaussian (log link)",
#         link = "log",
#         reason = paste(
#           "Gaussian with an log link was specified for",
...
#   }
# 
#   if (identical(family_code, "tweedie")) {
#     return(
#       list(
#         family = glmmTMB::tweedie(link = "log"),
#         label = "Tweedie (log link)",
#         link = "log",
#         reason = paste(
#           "Tweedie with a log link was specified for all",
#           "non-richness metrics"
#         )
#       )
#     )
#   }
...
#     "Unknown family code: ",
#     family_code,
#     ". Use either 'gaussian' or 'tweedie'."
#   )
# }
  ...
  if (identical(family_code, "poisson")) {
    return(
      list(
        ...
        link = "log",
        reason = paste(
          "Poisson with a log link was specified for",
          "species-richness count metrics"
        )
      )
    )
  }
  ...
  if (identical(family_code, "tweedie")) {
    return(
      list(
        ...
        link = "log",
        reason = paste(
          "Tweedie with a log link was specified for",
          "non-richness metrics"
        )
      )
    )
...
get_tweedie_power <- function(model, family_code) {
  if (is.null(model) || !identical(family_code, "tweedie")) {
    return(NA_real_)
  }
  
  tryCatch(
    as.numeric(glmmTMB::family_params(model)[1]),
    error = function(e) NA_real_
  )
}

fit_fixed_family_model <- function(
    formula,
    data,
    area_name,
    metric_name,
  ...
  family_details <- get_family_details(family_code)
  
  model_fit <- fit_glmmTMB_safely(
    formula = formula,
    data = data,
    family_object = family_details$family
  )
  
  model_check <- assess_model_fit(model_fit$model)
  
  dispersion_parameter <- if (is.null(model_fit$model)) {
    NA_real_
  } else {
    tryCatch(
      as.numeric(sigma(model_fit$model)),
      error = function(e) NA_real_
    )
  }
  
  tweedie_power <- get_tweedie_power(
    model = model_fit$model,
    family_code = family_code
  )
  
  model_error <- if (model_check$valid) {
    NA_character_
  } else {
    combine_messages(
      if (!is.na(model_fit$error)) {
        paste0("Model error: ", model_fit$error)
      },
      if (!model_check$converged) {
        "Model did not converge"
      },
      if (!model_check$pdHess) {
        "Model Hessian was not positive definite"
      },
      if (!model_check$finite_standard_errors) {
        "Model standard errors were not finite"
      },
      if (!is.finite(model_check$AIC)) {
        "Model AIC was not finite"
      }
    )
  }
  ...
  diagnostics <- tibble(
    ...
    family_code = family_code,
    selected_family = family_details$label,
    link = family_details$link,
    selection_reason = family_details$reason,
    selected_model_valid = model_check$valid,
    selected_converged = model_check$converged,
    selected_pdHess = model_check$pdHess,
    selected_finite_standard_errors =
      model_check$finite_standard_errors,
    selected_AIC = model_check$AIC,
    selected_logLik = model_check$logLik,
  ...
  list(
    model = if (model_check$valid) model_fit$model else NULL,
    candidate_model = model_fit$model,
    valid = model_check$valid,
    family_code = family_code,
    family_label = family_details$label,
    family_link = family_details$link,
    selection_reason = family_details$reason,
...
fit_one_region <- function(
  ...
  period_fit <- fit_fixed_family_model(
    formula = period_form,
    data = df,
    area_name = area_name,
  ...
  if (nrow(temporal_df) == 0) {
    ...
    temporal_fit <- fit_fixed_family_model(
...
# Species-richness metrics: Gaussian with log link.
...
# All remaining metrics: Tweedie with log link.
...
blank_panel <- function(
    panel_letter,
    label = "Model result unavailable"
) {
  ggplot() +
    annotate(
      "text",
...
reef_dat %>%
  ...
  summarise(
    n_samples = n(),
    n_zero = sum(n_species_sample == 0, na.rm = TRUE),
    percent_zero = 100 * mean,
    n_species_sample == 0,
    na.rm = TRUE
  )
)
...
reef_models$zero_summary %>%
  filter(reporting_name %in% "Aldinga - Aldinga Reef Sanctuary Zone")

reef_models$outputs[["Aldinga - Aldinga Reef Sanctuary Zone"]]$period_error



region_name <- "Bay of Shoals - Bay of Shoals Sanctuary Zone"

fish200_model <-
  fish_200_models$outputs[[region_name]]$period_candidate_model

abundance_model <-
  abund_models$outputs[[region_name]]$period_candidate_model

summary(fish200_model)
summary(abundance_model)

fish200_model$sdr$pdHess
abundance_model$sdr$pdHess

glmmTMB::diagnose(fish200_model)
glmmTMB::diagnose(abundance_model)
...
fish_200_dat %>%
  ...
  summarise(
    ...
    n_zeros = sum(total_abundance_sample == 0),
    percent_zero = mean(total_abundance_sample == 0) * 100,
    mean_value = mean(total_abundance_sample),
    maximum = max(total_abundance_sample),
    .groups = "drop"
  )


bay_abundance_poisson <- glmmTMB::glmmTMB(
  ...
  data = abund_dat %>%
    filter(
      reporting_name ==
        "Bay of Shoals - Bay of Shoals Sanctuary Zone"
    ),
  family = poisson(link = "log")
)

summary(bay_abundance_poisson)
bay_abundance_poisson$sdr$pdHess
VarCorr(bay_abundance_poisson)


bay_fish200_poisson <- glmmTMB::glmmTMB(
  total_abundance_sample ~ Period * Status +
    (1 | start_date_fct) +
    (1 | uwa_site_code),
  data = fish_200_dat %>%
...
summary(bay_fish200_poisson)
bay_fish200_poisson$sdr$pdHess
VarCorr(bay_fish200_poisson)

library(DHARMa)

fish200_res <- simulateResiduals(
  bay_fish200_poisson,
  n = 1000
)

plot(fish200_res)
testDispersion(fish200_res)
testZeroInflation(fish200_res)
